/*
** $Id: common.c 426 2008-07-21 02:42:12Z liupeng $
**
...
*/		} else
        ...
        if(strlen(stroke_window->str)!=SW_STR_LEN-2)
        	append(stroke_window->str, ch, SW_STR_LEN - 1, flag);
        else
        	return;
...
static BOOL get_substr_pos_ex(HDC hdc, const char* str, vw_element_t* e, 
		RECT *rc, int* len, RECT* pre_elem_bound)
{
	...
	*len = strlen(substr);
    ...
    SetRect(rc, pre_elem_bound->right + textsize.cx , pre_elem_bound->top, 
           pre_elem_bound->right + textsize.cx + cur_size.cx, 
			pre_elem_bound->bottom);
...
void vw_set_elements(struct _view_window_t* view_window, HWND hwnd)
{
    ...
    for (i=0; i<view_window->element_num; i++ ) {
        ...
        if(i==0){
            CopyRect(&pre_elem_bound, &r);
            pre_elem_bound.right=r.left+3;
        }else{
            CopyRect(&pre_elem_bound, &view_window->elements[i-1].bound);
        }
        get_substr_pos_ex(hdc, view_window->buffer, &view_window->elements[i], 
            &view_window->elements[i].bound, &view_window->elements[i].len, &pre_elem_bound);
#if 0
